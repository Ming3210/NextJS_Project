...
export default function page() {
  ...
  const [loadingProfile, setLoadingProfile] = useState<boolean>(false); // Loading state for profile navigation
  const [notification, setNotification] = useState<boolean>(false); // Loading state for notification
  ...
  const currentUser = useMemo(() => findUserById(user.id), [user.id, findUserById]);
  // Update search results with 1-second delay when input changes
  useEffect(() => {
    if (inputValue === "") {
      setSearchFilter([]);
      setLoading(false); // No need to show loader if input is empty
      return;
    }

    setLoading(true); // Show loader while waiting
    const delayDebounceFn = setTimeout(() => {
      const filteredUsers = state.client.userList?.filter((user: any) =>
        user.username.toLowerCase().includes(inputValue.toLowerCase())
      );
      setSearchFilter(filteredUsers || []);
      setLoading(false); // Stop loader after filtering
    }, 1000); // Delay of 1 second

    return () => clearTimeout(delayDebounceFn); // Clean up timeout if input changes
  }, [inputValue, state.client.userList]);

  const searchUser = (e: React.ChangeEvent<HTMLInputElement>) => {
    setInputValue(e.target.value.trim()); // Trim whitespace
  };

  // Delay for 1 second before navigating to the selected profile
  const profile = (user: any) => {
    setLoadingProfile(true); // Start full-screen loading
    setTimeout(() => {
      setLoadingProfile(false); // Stop loading after delay
      router.push(`/profile?id=${user.id}`); // Navigate to the user's profile
  ...
  // Memoized function to close the notification
  ...
  return (
   <div className="flex w-full items-center sticky top-0 z-10">
     ...
     <header className="header flex w-full justify-between h-16 border-solid border-0 border-b-4 bg-white border-b-red-600 ">
      {/* Left */}
      <div className="flex justify-between items-center">
        <div className="header__left flex w-full items-center ml-5">
          {/* Facebook Logo */}
          <svg
            viewBox="0 0 36 36"
          ...
          {/* Search Box */}
          <div className="header__searchbox flex items-center ml-4">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-6 relative left-8 w-6"
              width="1.25rem"
              height="1.5rem"
              fill="none"
              viewBox="0 0 24 24"
            ...
            <input
              ...
              value={inputValue}
              onChange={searchUser}
              className="header__searchinput border-2 pl-10 pr-4 border-gray-300 rounded-full h-10 w-[250px] py-2"
        ...
        {/* Search Results */}
        {inputValue && loading && (
          <div className="absolute top-12 left-0 w-[250px] bg-white border border-gray-300 rounded-lg p-4 text-gray-500">
            Loading...
          </div>
        )}
        {inputValue && !loading && searchFilter.length > 0 && (
          <div className="absolute top-12 left-0 w-[250px] bg-white border border-gray-300 rounded-lg">
            {searchFilter.map((item: any, index: number) => (
              <div
                key={index}
                onClick={() => profile(item)}
                className="p-2 flex items-center hover:cursor-pointer gap-4 border-b border-gray-200"
              >
                <img
                  src={item.avatar}
                ...
                {/* Fallback if `item.name` is unavailable */}
              </div>
            ))}
          </div>
        )}
      ...
      {/* Center */}
      <div className="flex center-header items-center w-[35%]">
        <div className="header__center w-full">
          <div className="header__iccontainer flex justify-between">
            <HeaderIcon
              active
              icon={
                <svg
      ...
      <div className="flex w-[20%] items-center justify-end">
        <div className="header__right  flex gap-5 left-header">
          <HeaderRightIcon
            icon={
              <i
          ...
          />
         {
          currentUser?.friendRequestsSent?  <div className="absolute right-[105px] bg-red-700 w-[20px] h-[20px] text-white rounded-full flex justify-center items-center">
       ...
       {notification && <Notification closeNotification={closeNotification} />}
          ...
          <div className="flex justify-center items-center mr-10">
            <img
              onClick={login}
              src="https://th.bing.com/th/id/OIP.z3fa8PjEnvzg4bhW61tEOwAAAA?rs=1&pid=ImgDetMain"
